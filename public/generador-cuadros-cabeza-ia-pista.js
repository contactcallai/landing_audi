...
async function generarPrimeraRonda(horariosPorPista, inscripciones, cabezasDeSerie = {}, intentos = 1000) {
    const grupos = {};
    const categoriasIA = {};
    ...
    const restriccionesJSON = await callOpenAI(prompt);
...
async function callOpenAI(prompt) {
    ...
    if (!apiKey) {
        console.error("❌ Error: No se ha encontrado la clave API de OpenAI en las variables de entorno.");
        return {};
    }
